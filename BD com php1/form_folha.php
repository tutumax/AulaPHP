...
if(isset($_REQUEST['enviar'])){
    ...
    try{
        $sqlInsert = $conn->prepare("insert into tb_folha(id_funcionarios,nome,salariob,horas,valorh,dependentes,salariobru,salarioliq,Irenda,inss)
        values(:id_funcionarios,:nome,:salariob,:horas,:valorh,:dependentes,:salariobru,:salarioliq,:inss,:inss)");
...
<table border="1">
    ...
    <?php
        try{
        $sqlSelect = $conn -> prepare("select * from tb_folha");
        $sqlSelect->execute();
